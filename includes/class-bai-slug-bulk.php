...
class BAI_Slug_Bulk {
    ...
    public function enqueue( $hook ) {
        ...
        wp_localize_script( 'bai-slug-bulk', 'BAISlugBulk', [
            ...
            'i18n'     => [
                ...
                'done' => ( BAI_Slug_I18n::lang() === 'en' ? 'Done' : '任务完成' ),
                'network_error' => ( BAI_Slug_I18n::lang() === 'en' ? 'Network error' : '网络错误' ),
                'cursor_reset' => ( BAI_Slug_I18n::lang() === 'en' ? 'Cursor reset' : '游标已重�? ),
            ],
        ] );
    }
    ...
    public function ajax_batch() {
        ...
        $post_types = isset( $_POST['post_types'] ) && is_array( $_POST['post_types'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['post_types'] ) ) : [ 'post', 'page' ];
        ...
        $sql = $wpdb->prepare(
            "SELECT ID, post_title, post_name, post_type FROM {$wpdb->posts}
             ...
             ORDER BY ID ASC
             LIMIT %d",
            array_merge( $statuses, $post_types, [ (int) $cursor, (int) $scan_limit ] )
        );
        ...
        foreach ( $rows as $row ) {
            ...
            if ( $only_non_ascii ) {
                if ( $current_slug !== '' && ! $this->has_non_ascii( $current_slug ) ) continue;
            }
            if ( $only_eq_sanitized ) {
                if ( $current_slug !== '' && $current_slug !== $sanitized_title ) continue;
            }
            ...
            if ( $slug ) {
                ...
                if ( is_wp_error( $res ) ) {
                    BAI_Slug_Log::add( '批量更新失败�? . $res->get_error_message(), $row->post_title );
                } else {
                ...
                BAI_Slug_Log::add( '批量生成失败（无结果）�?, $row->post_title );
            }
